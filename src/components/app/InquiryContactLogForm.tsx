...
export function InquiryContactLogForm({ inquiryId }: { inquiryId: string }) {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/admin/inquiries/${inquiryId}/contacts`, {
        method: "POST",
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-2">
      ...
      <textarea
        ...
        disabled={saving}
        rows={2}
        placeholder="연락 결과 (예: 통화 연결, 관심 확인)"
