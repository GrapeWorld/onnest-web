...
export function PartnerRequestNoteForm({ requestId }: { requestId: string }) {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/partner/service-requests/${requestId}/notes`, {
        method: "POST",
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-2">
      <textarea
        ...
        disabled={saving}
        rows={3}
        placeholder="이 요청에 대한 내부 메모를 남겨주세요. (고객·관리자에게는 노출되지 않습니다)"
