...
export function AdminExportSearchBar({ initialQuery }: { initialQuery: string }) {
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    router.push(trimmed ? `/admin/exports?q=${encodeURIComponent(trimmed)}` : "/admin/exports");
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-3 md:grid-cols-[1fr_auto] min-w-0">
      <input
        ...
        placeholder="내보낼 고객의 이름, 이메일, 휴대폰 번호로 검색"
